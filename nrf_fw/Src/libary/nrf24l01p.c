...
static uint8_t write_register_arr(uint8_t reg, uint8_t* value,uint16_t size)
{
    uint8_t command = NRF24L01P_CMD_W_REGISTER | reg;
    uint8_t status;
    //uint8_t write_val = value;
...
void nrf24l01p_tx_irq(uint8_t *rxPayload)
{
    uint8_t sta = nrf24l01p_get_status();
    ...
    if (sta&RX_OK){
        nrf24l01p_read_rx_fifo(rxPayload);
        nrf24l01p_flush_rx_fifo();
        nrf24l01p_clear_rx_dr();
        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
    }
...
uint8_t tx_ack_address[5]={0x11,0xC2,0xC2,0xC2,0xC2};
